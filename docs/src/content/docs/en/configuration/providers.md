...
4. For requests sent to the official OpenCode Go endpoint, CCR forwards `x-claude-code-session-id` (or the legacy `x-claude-session-id`) as `x-opencode-session`. If the client supplies neither header, CCR uses the request's `metadata.user_id` when present and otherwise a stable `ccr-<uuid>` generated once per process; explicit `x-opencode-session` configuration takes precedence. This header is not added to Zen or unrelated providers.

If Go is locked, or does not appear as an import option, connect `OpenCode Go` in OpenCode first and let OpenCode refresh its local model cache, then return to CCR and rescan.
