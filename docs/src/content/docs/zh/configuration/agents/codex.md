...
- **App 提示缺少 application network requirements**：更新 CCR 后重新打开 App。CCR 会向 Codex App 明确声明该入口不需要额外的应用网络域名。
- **已允许写入但任务仍显示只读**：沙箱模式与审批方式是两项独立设置；聊天中输入“允许写入”不会切换沙箱。请在任务权限中选择 **工作区可写**。CCR 默认把第三方路由下的工具审批交给用户，避免自动审查器超时；只有明确需要恢复原生自动审查时，才在配置的环境变量中设置 `CCR_CODEX_ALLOW_AUTO_REVIEW=1` 并重新打开 App。
