...
试用阶段建议选择 **仅从 CCR 打开时生效**，并且总是从 CCR 打开 Agent。这样配置只影响 CCR 启动的实例，不会改掉你系统里原本直接打开的 Claude Code、Codex、Grok CLI、Kimi CLI 或 ZCode。Claude Code 默认保持 **CCR 隔离配置**；只有明确希望 CCR 启动的 CLI 共享现有配置目录时，才选择 **复用现有 Claude 配置**。
