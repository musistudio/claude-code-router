...
export class GroupCommand {
   ...
   */
  private static async createGroupConfig(): Promise<RouteGroup> {
    const configFile = loadConfig();
    const groupConfig: RouteGroup = { default: '' };
    ...
    while (true) {
      ...
      if (type === '__done__') {
        ...
        const validation = RouteGroupManager.validateGroupConfig(groupConfig, configFile.Providers);
        if (!validation.isValid) {
          console.log(`${RED}Invalid configuration:${RESET}`);
          validation.invalidConfigs.forEach(item => console.log(`  - ${item}`));
          continue;
        }
        return groupConfig;
      }
