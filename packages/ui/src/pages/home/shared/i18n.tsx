...
export const appCopy: Record<ResolvedLanguage, AppCopy> = {
  en: {
    ...
    text: {
      ...
      "Default: drop summaries": "Default: drop summaries",
      "Do not send": "Do not send",
      "Do not send reasoning history (safest)": "Do not send reasoning history (safest)",
      "Drop summaries": "Drop summaries",
      "Handled automatically by CCR protocol rules: complete signed thinking blocks replay verbatim (official requirement); same-endpoint and same-account checks are CCR-added isolation; switching models drops old thinking per official rules; an unrecoverable active tool turn returns an error. No configuration needed.": "Handled automatically by CCR protocol rules: complete signed thinking blocks replay verbatim (official requirement); same-endpoint and same-account checks are CCR-added isolation; switching models drops old thinking per official rules; an unrecoverable active tool turn returns an error. No configuration needed.",
      "Handled automatically by CCR protocol rules: only official signed thought/tool steps replay; cross-model replay between Gemini models on the same service is officially supported (same-endpoint and same-account checks are CCR-added isolation). No configuration needed.": "Handled automatically by CCR protocol rules: only official signed thought/tool steps replay; cross-model replay between Gemini models on the same service is officially supported (same-endpoint and same-account checks are CCR-added isolation). No configuration needed.",
      "Handled automatically by CCR protocol rules: thought signatures replay when the model matches and the signature is complete (same-endpoint and same-account checks are CCR-added isolation); a native Gemini 3 call that lost its signature rejects the active tool group and drops closed groups. No configuration needed.": "Handled automatically by CCR protocol rules: thought signatures replay when the model matches and the signature is complete (same-endpoint and same-account checks are CCR-added isolation); a native Gemini 3 call that lost its signature rejects the active tool group and drops closed groups. No configuration needed.",
      "Handled automatically: readable reasoning is mapped to the fields supported by each vendor adapter; native structures replay only when the vendor adapter explicitly supports them, the state is valid, and the route is unchanged; otherwise they are removed. No configuration needed.": "Handled automatically: readable reasoning is mapped to the fields supported by each vendor adapter; native structures replay only when the vendor adapter explicitly supports them, the state is valid, and the route is unchanged; otherwise they are removed. No configuration needed.",
      "Native history": "Native history",
      "Native history (including encrypted reasoning)": "Native history (including encrypted reasoning)",
      "Reasoning history": "Reasoning history",
      "Reasoning history compatibility": "Reasoning history compatibility",
      "Reasoning summary": "Reasoning summary",
      "Replay readable reasoning": "Replay readable reasoning",
      "Replay readable reasoning (compatible services)": "Replay readable reasoning (compatible services)",
      "map vendor reasoning fields": "map vendor reasoning fields",
      "replay signed steps": "replay signed steps",
      "replay signed thinking": "replay signed thinking",
      "replay thought signatures": "replay thought signatures",
      "Send summaries as reasoning text": "Send summaries as reasoning text",
      "Stripping may discard compacted context. Select this only if context loss is acceptable.": "Stripping may discard compacted context. Select this only if context loss is acceptable.",
  ...
  zh: {
    ...
    text: {
      ...
      "Default: drop summaries": "默认：丢弃摘要",
      "Do not send": "不发送",
      "Do not send reasoning history (safest)": "不发送 reasoning 历史（最安全）",
      "Drop summaries": "丢弃摘要",
      "Handled automatically by CCR protocol rules: complete signed thinking blocks replay verbatim (official requirement); same-endpoint and same-account checks are CCR-added isolation; switching models drops old thinking per official rules; an unrecoverable active tool turn returns an error. No configuration needed.": "由 CCR 按协议规则自动安全处理：采集完整时原样回放带签名 thinking（官方要求）；同端点、同账号为 CCR 附加隔离；切换模型按官方规则移除旧 thinking；活动工具轮无法安全续接时返回错误。此协议无需配置。",
      "Handled automatically by CCR protocol rules: only official signed thought/tool steps replay; cross-model replay between Gemini models on the same service is officially supported (same-endpoint and same-account checks are CCR-added isolation). No configuration needed.": "由 CCR 按协议规则自动安全处理：仅回放官方带签名的 thought/tool step；同服务下跨 Gemini 模型回放为官方支持（同端点、同账号为 CCR 附加隔离）。此协议无需配置。",
      "Handled automatically by CCR protocol rules: thought signatures replay when the model matches and the signature is complete (same-endpoint and same-account checks are CCR-added isolation); a native Gemini 3 call that lost its signature rejects the active tool group and drops closed groups. No configuration needed.": "由 CCR 按协议规则自动安全处理：同模型且签名完整时回放 thoughtSignature（同端点、同账号为 CCR 附加隔离）；Gemini 3 原生签名丢失时拒绝活动工具组、整组删除历史组。此协议无需配置。",
      "Handled automatically: readable reasoning is mapped to the fields supported by each vendor adapter; native structures replay only when the vendor adapter explicitly supports them, the state is valid, and the route is unchanged; otherwise they are removed. No configuration needed.": "自动处理：目标厂商适配器明确支持时，将可读 reasoning 映射到该厂商支持的字段；原生结构仅在厂商适配器明确支持、状态合法且路由未变化时回放；否则删除。此协议无需配置。",
      "Native history": "原生历史",
      "Native history (including encrypted reasoning)": "原生历史（含加密推理）",
      "Reasoning history": "推理历史",
      "Reasoning history compatibility": "推理历史兼容",
      "Reasoning summary": "推理摘要",
      "Replay readable reasoning": "回放可读 reasoning",
      "Replay readable reasoning (compatible services)": "回放可读 reasoning（兼容服务）",
      "map vendor reasoning fields": "映射厂商 reasoning 字段",
      "replay signed steps": "回放签名 step",
      "replay signed thinking": "回放签名 thinking",
      "replay thought signatures": "回放 thoughtSignature",
      "Send summaries as reasoning text": "将摘要作为 reasoning 文本发送",
      "Stripping may discard compacted context. Select this only if context loss is acceptable.": "删除推理历史可能丢失已压缩上下文。仅在可以接受上下文丢失时选择此项。",
