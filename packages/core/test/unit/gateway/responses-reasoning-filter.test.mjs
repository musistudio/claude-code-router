...
test("native Responses requests keep reasoning items without encrypted_content", () => {
  const clientReasoning = {
    content: [{ text: "client supplied reasoning", type: "reasoning_text" }],
    id: "rs_68c0f2a41b3d4e5f9a0b1c2d3e4f5a6b",
    summary: [{ text: "step one", type: "summary_text" }],
    type: "reasoning"
  };
  const input = responsesInput(
    [
    ...
    { sourceAdapterKey: "openai_responses", sourceProvider: "openai" }
  );
  ...
  assert.deepEqual(result.body.input[1], clientReasoning);
});

test("Chat Completions conversions keep reasoning items without encrypted_content", () => {
  const input = responsesInput([thinkingDerivedReasoningItem()], {
  ...
  assert.equal(result, input.upstreamRequest);
  assert.equal(result.body.input.length, 1);
});

test("requests without a known source protocol are left unchanged", () => {
  const input = responsesInput([thinkingDerivedReasoningItem()], {
...
test("gateway boundary plugin registers the reasoning filter hook", async () => {
  ...
  const nativeInput = responsesInput([
    thinkingDerivedReasoningItem(0),
    { content: [{ text: "hello", type: "input_text" }], role: "user", type: "message" }
  ], { sourceAdapterKey: "openai_responses", sourceProvider: "openai" });
  const nativeResult = await filterHook.transformRequest(nativeInput);

  assert.equal(nativeResult.ok, true);
  assert.equal(nativeResult.value, nativeInput.upstreamRequest);
  assert.deepEqual(nativeResult.value.body.input.map((item) => item.type), ["reasoning", "message"]);
});
